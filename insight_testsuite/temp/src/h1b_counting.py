...
def get_top10(inputList):
    """
    get top 10 most common element
    :param inputList: a list
    :return: top 10 element with case number and percentage
    """
    total_num_element = len(inputList)
    key_count = Counter(inputList)
    key_count = sorted(key_count.iteritems(), key=lambda (k, v): (-v, k))
...
#python ./src/h1b_counting.py ./insight_testsuite/tests/test_3_2015data/input/h1b_input.csv ./output/top_10_occupations.txt ./output/top_10_states.txt
